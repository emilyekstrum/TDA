...
To check GPU availability: ```import torch``` & ```torch.cuda.is_available()``` -> if ```False``` you may not be able to handle CEBRA models. There may be a CPU work-around, but that has not been tested with this pipeline.
